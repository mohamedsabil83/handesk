@if($ticket->isEscalated() )
    @if($ticket->status < App\Ticket::STATUS_CLOSED)
        ...
        <div class="p4 bg-danger white mt-4 mb4 br1 clear-both">
            ...
            @icon(flag fas) {!! __('ticket.escalatedDesc') !!}
            <button class="primary ml2">@icon(flag fas) {{ __('ticket.de-escalate') }}</button>
            {{ Form::close() }}
        </div>
    @endif
@else
    ...
    <button class="secondary">@icon(flag fas) {{ __('ticket.escalate') }}</button>
    {{ Form::close() }}
    </div>
@endif
