    ...
    @if(auth()->user()->assistant || auth()->user()->admin)
        @if($ticket->getIssueId())
            <div class="float-right mt-1 mr4 ml-3">
                <a class="button fs2 secondary" href="{{$ticket->issueUrl()}}" target="_blank"> @icon(bug fas) {{ __('ticket.seeIssue') }}</a>
            </div>
        @else
            @if($ticket->subject && $ticket->summary)
                <div class="float-right mt-2 mr4 ml-3">
                <button class="secondary dropdown"> @icon(bug fas) {{ __('ticket.createIssue') }}</button>
            ...
            @else
                <div class="float-right mt-2 mr4 ml-3">
                    <button class="secondary"> @icon(bug fas) {{ __('ticket.needSubject') }}</button>
                </div>
            @endif
        @endif
    @endif
