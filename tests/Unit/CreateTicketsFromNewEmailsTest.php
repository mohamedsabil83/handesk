...
class CreateTicketsFromNewEmailsTest extends TestCase
{
    ...
    public function does_create_comments_for_new_comment_emails(){
        Notification::fake();
        $fakePop = new FakeMailbox();
        $agent                              = User::factory()->create(["email" => "tony@stark.com"]);
        $ticketWithCommentRequester         = Requester::factory()->create(["email" => "peter@parker.com"]);
        $ticketThatWillGetTheCommentByMail  = Ticket::factory()->create(["id" => 18, "requester_id" => $ticketWithCommentRequester ]);
    ...
    public function a_comment_from_requester_email_changes_status_to_open(){
        ...
        $agent                              = User::factory()->create(["email" => "tony@stark.com"]);
        $ticketWithCommentRequester         = Requester::factory()->create(["email" => "peter@parker.com"]);
        $ticketThatWillGetTheCommentByMail  = Ticket::factory()->create(["id" => 18, "requester_id" => $ticketWithCommentRequester, "status" => Ticket::STATUS_SOLVED ]);
